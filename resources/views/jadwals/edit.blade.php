                ...
                <div class="p-6 text-gray-900">
                    <form action="{{ route('jadwals.update', $jadwal->id) }}" method="POST">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="judul_video" name="judul_video" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('judul_video', $jadwal->judul_video) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <select name="menko_id" id="menko_id" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">
                                ...
                                @foreach($menkos as $item)
                                    {{-- Logika 'selected' ini akan memilih konten yang sudah terhubung --}}
                                    <option value="{{ $item->id }}" {{ old('menko_id', $jadwal->menko_id) == $item->id ? 'selected' : '' }}>
                                        {{ $item->judul }}
                                    </option>
                                @endforeach
                            </select>
                        ...
                        <div class="mb-4">
                            ...
                            <input type="datetime-local" id="tanggal_tayang" name="tanggal_tayang" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('tanggal_tayang', $jadwal->tanggal_tayang ? \Carbon\Carbon::parse($jadwal->tanggal_tayang)->format('Y-m-d\TH:i') : '') }}">
                        ...
                        <div class="mb-4">
                            ...
                            <input type="text" id="link_video" name="link_video" class="block mt-1 w-full rounded-md shadow-sm border-gray-300" value="{{ old('link_video', $jadwal->link_video) }}">
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="deskripsi" name="deskripsi" rows="5" class="block mt-1 w-full rounded-md shadow-sm border-gray-300">{{ old('deskripsi', $jadwal->deskripsi) }}</textarea>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            ...
                            <button type="submit" class="ms-4 inline-flex items-center px-4 py-2 bg-gray-800 border border-transparent rounded-md font-semibold text-xs text-white uppercase tracking-widest">UPDATE</button>
                        </div>
                    </form>
                </div>
            </div>
